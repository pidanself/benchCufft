...
int main(int argc, char **argv) {
    ...
    cufftPlan1d(&fft_plan, signal_length, CUFFT_C2C, 1);
    ...
    for (int i = 0; i < meta_trials; i++) {
        ...
        float elapsed_time_sec = elapsed_time_ms;
        sum_of_elapsed_times += elapsed_time_sec;
    }
    printf("[INFO] Finished!\n");
    printf("[INFO] Average: %lf ms\n", sum_of_elapsed_times / (meta_trials * fft_trials));
    printf("[INFO] Average Gflops: %lf Gflops\n", 5*(1e-9)*signal_length*log2(signal_length)/(sum_of_elapsed_times / (meta_trials * fft_trials*1000)));
