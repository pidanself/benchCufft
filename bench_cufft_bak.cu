...
int main(int argc, char **argv) {
    ...
    cufftComplex *h_original_signal, *h_output;
    cudaMallocHost((void **) &h_original_signal, sizeof(cufftComplex) * signal_length);
    cudaMallocHost((void **) &h_output, sizeof(cufftComplex) * signal_length);
    ...
    for (int i = 0; i < meta_trials*10000; i++) {
    	...
    	cudaMemcpy(h_output, d_applied_fft_signal, sizeof(cufftComplex) * signal_length, cudaMemcpyDeviceToHost);
        ...
        float elapsed_time_sec = elapsed_time_ms/1000;
        sum_of_elapsed_times += elapsed_time_sec;
        //printf("%f sec\n", elapsed_time_sec);
    }
    printf("[INFO] Finished!\n");
    printf("[INFO] Average: %lf sec\n", sum_of_elapsed_times / meta_trials/10000);
